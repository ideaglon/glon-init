...
var util = {
  ...
  makeFiles: function(sourceDir, targetDir, data, filter, callback, cwd) {
    ...
    fetchTpl: function(url, callback, force) {
        ...
        var now = Date.now();
        var basedir = path.join(os.homedir(), '.glon', 'init', 'templates');
...
function readAliasFile() {
    var aliasFile = path.join(os.homedir(), '.glon', 'init', 'template.json');
    try {
      return JSON.parse(fs.readFileSync(aliasFile, 'utf-8'));
    } catch(e) {
      return writeAliasFile(template);
    }
}

function writeAliasFile(template) {
    var configDir = path.join(os.homedir(), '.glon', 'init');
    try {
      mkdirp.sync(configDir);
      fs.writeFileSync(path.join(configDir, 'template.json'), JSON.stringify(template));
    } catch(e) {
    }
    return template;
}